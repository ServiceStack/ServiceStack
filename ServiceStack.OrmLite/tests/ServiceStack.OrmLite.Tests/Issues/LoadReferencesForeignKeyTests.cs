...
[TestFixtureOrmLite]
public class LoadReferencesForeignKeyTests(DialectContext context) : OrmLiteProvidersTestBase(context)
{
    [Test]
    ...
    public void Can_create_tables_with_multiple_references()
    {
        ...
        using (var db = OpenDbConnection())
        {
            user.Branches = [new() { UserId = user.Id }];
            user.Addresses = [new() { UserId = user.Id }];
