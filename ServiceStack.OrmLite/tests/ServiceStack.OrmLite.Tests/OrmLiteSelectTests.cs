...
public partial class OrmLiteSelectTests : OrmLiteProvidersTestBase
{
    ...
    [Test]
    public void Disable_AllowEscapeWildcards_ok()
    {
        using var db = OpenDbConnection();
        db.DropAndCreateTable<EscapeWildcardsModel>();
        ...
        var q = db.From<EscapeWildcardsModel>(x =>
            {
                x.AllowEscapeWildcards = true;
            })
            .Where(x => x.Index.StartsWith("1_2"));
        var sql1 = q.ToMergedParamsSelectStatement();
        Debug.Assert(sql1.Contains("like '1^_2%' escape '^'"));
        var c1 = db.Count(q);
        Debug.Assert(c1 == 2);
        ...
        Debug.Assert(sql2.Contains("like '1_2%'"));
        var c2 = db.Count(q);
        Debug.Assert(c2 == 3);
    }
}