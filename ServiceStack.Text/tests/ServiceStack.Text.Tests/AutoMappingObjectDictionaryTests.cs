...
public class AutoMappingObjectDictionaryTests
{
    ...
    [Test]
    public void Does_return_WithoutDefaultValues()
    {
        var dto = new DtoWithEnum { Name = "A", Color = Color.Blue };
        var obj = dto.ToObjectDictionary();
        Assert.That(obj.WithoutDefaultValues().Count, Is.EqualTo(2));
        Assert.That(obj.WithoutDefaultValues(ignoreKeys:new[]{ nameof(DtoWithEnum.Name) }).Count, Is.EqualTo(1));

        dto.Name = null;
        obj = dto.ToObjectDictionary();
        Assert.That(obj.WithoutDefaultValues().Count, Is.EqualTo(1));
        Assert.That(obj.WithoutDefaultValues(ignoreKeys:new[]{ nameof(DtoWithEnum.Color) }).Count, Is.EqualTo(0));

        dto.Color = Color.Red;
        obj = dto.ToObjectDictionary();
        Assert.That(obj.WithoutDefaultValues().Count, Is.EqualTo(0));
    }

    [Test]
    public void Does_return_HasNonDefaultValues()
    {
        var dto = new DtoWithEnum { Name = "A", Color = Color.Blue };
        var obj = dto.ToObjectDictionary();
        Assert.That(obj.HasNonDefaultValues());
        Assert.That(obj.HasNonDefaultValues(ignoreKeys:new[]{ nameof(DtoWithEnum.Name) }));
        Assert.That(!obj.HasNonDefaultValues(ignoreKeys:new[]{ nameof(DtoWithEnum.Name), nameof(DtoWithEnum.Color) }));
