/**
 * SchemaGrid - renders rows against a JSON Schema, the way DataGrid renders them against a
 * MetadataType. Interned so the page needs no App metadata: everything it formats with comes
 * from the schema's own `ui.format` / `ui.ref`.
 *
 *     <SchemaGrid :items="rows" :schema="Auto.model" :selected-columns="columns"
 *                 :is-selected="() => false" @row-selected="edit" @header-selected="filter">
 *         <template #header="{ column, label }">…</template>
 *     </SchemaGrid>
 *
 * Mirrors DataGrid's surface (items/selectedColumns/headerTitles/isSelected, rowSelected +
 * headerSelected, the `header` slot, striped rows) so it drops in where DataGrid was.
 *
 * Dependencies
 *   vue                   3.x
 *   @servicestack/vue     useFormatters().formatValue() - takes a FormatInfo directly and
 *                         needs no metadata, so it formats identically to CellFormat.
 *                         useConfig() for the fallback table icon, <Icon> to render it.
 *   @servicestack/client  mapGet(), for reading camelCase rows with PascalCase schema keys.
 *   ./useSchemas.mjs      only when a [Ref] declares no refLabel - the referenced Model's
 *                         schema is fetched to pick a sensible one.
 */
...
const SchemaGrid = {
    ...
    setup(props) {
        ...
        /**
         * The property holding the referenced row, so a [Ref] shows a label instead of an id.
         * A complex property carries its own row (Discount, PostalAddress); a scalar FK points
         * at a sibling complex property of the same Model (CouponId -> Discount).
         */
        ...
        /**
         * Which property labels the referenced row. Most [Ref]s name one; those that don't
         * (Booking.Discount) fall back to the referenced Model's first string property, which
         * needs its schema - the one case the grid fetches anything.
         */
        ...
        /**
         * The FormatInfo to render a value with. A date carrying no explicit [Format] still
         * reads better as a date than as a raw ISO string, so it falls back to Formats.date
         * (`formatDate`) - the same default the library applies elsewhere.
         */
        ...
        /** the referenced Model's [Icon], else the generic table icon LookupInput falls back to */
        ...
        /** a list of objects, which no single cell can show - PhoneNumbers, Attachments */
        ...
        /**
         * What to put in one cell: either `text` (with an optional `icon`) or formatted `html`,
         * plus the `title` shown on hover - the untruncated value, and the only place the full
         * contents of a summarised list can be read.
         */
