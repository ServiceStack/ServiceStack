/**
 * JsonView - renders an API response as something readable, the way HtmlFormat.vue does but
 * with the markup each shape actually calls for:
 *
 *     <JsonView :value="response" />
 *
 *   an object          <dl> - a list of terms and their descriptions, which is what an object is.
 *                      HtmlFormat uses a <table> with a <th> per row, so every object claims to
 *                      be tabular data with one unnamed column.
 *   a list of objects  <table> with <thead>, scoped headers and a <caption> saying how many rows
 *   a list of scalars  <ol> of chips - ordered, because the index is part of the value's identity
 *   a date             <time datetime="…"> carrying the machine-readable value it was formatted from
 *   a url / email      a real link; an image url renders the image
 *   null               said out loud, rather than rendered as the empty string it isn't
 *
 * Deeply nested objects fold into <details> instead of pushing the page sideways.
 *
 * Dependencies
 *   vue                   3.x
 *   @servicestack/vue     useFormatters().formatValue(), so dates and numbers read the same
 *                         here as they do in the data grids
 *   @servicestack/client  humanify() for labels, uniqueKeys() for a table's column set
 */
...
/** Fold past this depth, so one deeply nested field can't push everything else off screen */
const FOLD_DEPTH = 3
