/**
 * SchemaResults - a results grid and everything that queries it: paging, per-column sort and
 * filter popups, query preferences and the active-filter chips.
 *
 *     <SchemaResults :schema="Auto" v-model:query="query" selectable @row-selected="edit">
 *         <template #toolbar><PrimaryButton>New</PrimaryButton></template>
 *     </SchemaResults>
 *
 * `schema` is a /auto/{Model}.json envelope - only `name`, `query` and the row schema are read,
 * so this works for the page's own model and for a [Ref] model being picked in a lookup. The
 * rows are described by `viewModel` when the Query API is an IQueryDb<From,Into>, else `model`.
 *
 * What's being queried lives in one object, `{ filters, orderBy, skip }`, exposed as
 * `v-model:query`. AutoQuerySchema binds that to the URL so every view is linkable; a lookup
 * picker binds nothing and lets it stay component-local. Leaving the binding off is what makes
 * a second, nested grid possible without the two fighting over the same query string.
 *
 * Dependencies
 *   vue                   3.x
 *   vue-router            no - the query model is plain data, the host decides where it lives
 *   @servicestack/vue     FilterColumn, QueryPrefs, ErrorSummary (globally registered by the
 *                         plugin), useMetadata() for filterDefinitions only - which falls back
 *                         to AutoQuery's built-in conventions when no metadata is loaded
 *   @servicestack/client  mapGet()
 *   ./SchemaGrid.mjs      renders the rows
 *   ./useSchemas.mjs      rowSchema(), which picks `viewModel` over `model`
 *
 * Query preferences (visible columns, page size) persist to localStorage under `prefsKey`.
 */
...
const SchemaResults = {
    ...
    setup(props, { emit, expose }) {
        ...
        /** every change replaces the whole query, so removing a filter is as simple as adding one */
        ...
        /**
         * <FilterColumn> expects a MetadataPropertyType, which came from App metadata. The JSON
         * Schema carries the same facts, and its `type` needs no translation - isString('string')
         * is already true, so string columns get the string conventions and the rest don't.
         * isEnum stays false: its checkbox list needs enumOptions() from metadata, so an enum
         * column filters through the normal operator UI instead.
         */
        const metaProp = name => {
