...
const SidebarNav = {
    template:/*html*/`
      <div>
          ...
          <nav class="mt-6 flex-1 px-3 bg-white space-y-1" aria-label="Sidebar">
            <a v-for="({id,label,icon}) in store.adminLinks" v-href="{ admin:id, $clear:true }"
               ...
               :class="[(routes.admin ?? '') === id ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900', 
                            'group flex items-center gap-x-3 px-2.5 py-2 text-base font-medium rounded-md transition-colors']">
              <Icon :image="icon" :class="[(routes.admin ?? '') === id ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500', 'size-[22px] shrink-0']" />
              {{ label }}
            </a>
          </nav>
      </div>
    ...
    template:/*html*/`
      ...
      <div class="flex items-center">
        ...
        <div class="ml-3 min-w-0">
          <p class="truncate text-base font-medium text-gray-700 group-hover:text-gray-900">
            {{ store.displayName }}
          </p>
        </div>
      </div>
