                    ...
                    <div class="flex items-start justify-between space-x-3">
                      <div class="space-y-1">
                        <h2 class="text-lg font-medium text-gray-900 flex">
                          <div v-if="selected.statusCode" :class="['mr-2 font-medium',statusColor(selected.statusCode)]">
                            {{ selected.statusCode }}
                          </div>
                          <div>
                            <span v-if="selected.httpMethod">{{ selected.httpMethod }}</span>
                            <a v-href="href({ pathInfo:selected.pathInfo })" class="text-blue-600 hover:text-blue-800">
                              {{ selected.pathInfo }}
                            </a>
                          </div>
                        </h2>
                        <div class="text-sm text-gray-500 flex flex-wrap">
                          ...
                          <div v-if="selected.ipAddress" class="ml-2 flex items-center" title="IP Address">
                            ...
                            <a v-href="href({ ipAddress:selected.ipAddress })" class="text-blue-600 hover:text-blue-800">
                              {{ selected.ipAddress }}
                            </a>
                          </div>
                          ...
                          <div v-if="selected.traceId" class="ml-2 flex items-center" title="Trace Id">
                            ...
                            <a v-href="href({ $page:'profiling', traceId:selected.traceId })" :title="selected.traceId"
                               class="text-blue-600 hover:text-blue-800">
                              trace request
                            </a>
                          </div>
                    ...
                    <div v-if="selected.requestDto" class="flex overflow-auto">
                      <div class="p-2 relative w-full">
                        ...
                        <div v-if="routes.body === ''" class="pt-2 icon-outer" style="min-height:2.5rem">
                          ...
                          <pre class="whitespace-pre-wrap"><code lang="json" v-highlightjs="selectedRequestDtoJson"></code></pre>
                        </div>
                    ...
                    <div v-if="selected.responseDto" class="flex overflow-auto">
                      <div class="p-2 relative w-full">
                        ...
                        <div v-if="routes.body === ''" class="pt-2 icon-outer" style="min-height:2.5rem">
                          ...
                          <pre class="whitespace-pre-wrap"><code lang="json" v-highlightjs="selectedResponseDtoJson"></code></pre>
                        </div>
                        <div v-else-if="routes.body === 'raw'" class="flex pt-2">
        ...
        return {
            ...
            get selectedHeaders() {
                ...
                return Object.keys(selected.value.headers).filter(x => ignoredHeaders.indexOf(x) < 0)
                    .reduce((acc,x) => { acc[x] = selected.value.headers[x]; return acc }, {})
            },
            get responseHeaders() {
                ...
                return Object.keys(selected.value.responseHeaders).filter(x => ignoredHeaders.indexOf(x) < 0)
                    .reduce((acc,x) => { acc[x] = selected.value.responseHeaders[x]; return acc }, {})
            },
            statusColor(status) {
