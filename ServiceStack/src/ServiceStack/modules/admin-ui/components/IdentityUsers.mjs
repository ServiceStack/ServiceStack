          ...
          <div v-if="analytics">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 w-full gap-2">
              <div v-if="mapCounts(analytics,'apis')">
                ...
                <div class="mt-1 bg-white rounded-lg shadow-sm ring-1 ring-gray-900/5 p-4" style="height:300px">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        <th v-for="name in fieldNames" scope="col" 
                            class="group cursor-pointer px-4 py-2.5 text-left text-xs font-semibold text-gray-600 tracking-wide whitespace-nowrap">
                    ...
                    <tr v-for="(row,index) in results" key="mapGet(row, 'Id')" @click="toggle(row)"
                        :class="['cursor-pointer', expanded(row.Id) ? 'bg-indigo-50' : 'hover:bg-gray-100 ' + (index % 2 === 0 ? 'bg-white' : 'bg-gray-50/60')]">
