...
public class DbJobsAdminServices(ILogger<DbJobsAdminServices> log, IBackgroundJobs jobs, IAutoQueryDb autoQuery) : Service
{
    ...
    public object Any(AdminJobDashboard request)
    {
        ...
        to.Queues = db.SqlList<JobStat>(db.From<JobSummary>()
            ...
            .And(dateFilter)
            .UnsafeGroupBy(groupByFields(nameof(JobSummary.Queue)))
            .Select(x => new {
                Name = x.Queue,
                x.State,
                Retries = Sql.Custom(sqlRetries),
    ...
    public object Any(AdminJobInfo request)
    {
        ...
        var to = new AdminJobInfoResponse
        {
            ...
            Capabilities = ["queues", "queue-controls", "priority", "retry-backoff", "idempotent-enqueue",
                ...
                "distributed-processing", "distributed-schedules", "leases", "automatic-failover",
                "fenced-completion"],
    ...
    /// <summary>
    /// SQL for the milliseconds a Job waited between being created and started. Date arithmetic
    /// has no portable form, so each dialect needs its own expression.
    /// </summary>
    public static string GetWaitMsSql(IDbConnection db)
    {
        ...
        {
            DbKind.Sqlite => $"(julianday({started}) - julianday({created})) * 86400000",
            DbKind.PostgreSql => $"EXTRACT(EPOCH FROM ({started} - {created})) * 1000",
            // DATEDIFF(MILLISECOND,..) overflows an int past ~24 days, so count seconds
            DbKind.SqlServer => $"DATEDIFF(SECOND, {created}, {started}) * 1000.0",
            DbKind.MySql => $"TIMESTAMPDIFF(MICROSECOND, {created}, {started}) / 1000.0",
            _ => $"DATEDIFF(SECOND, {created}, {started}) * 1000.0",
        };
    }
