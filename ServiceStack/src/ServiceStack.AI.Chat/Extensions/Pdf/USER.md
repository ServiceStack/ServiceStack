...
in the Admin UI writes to your project — generation is performed by a development-only Startup Task.
...
| Tab | What you get |
| --- | --- |
| **PDF Data Models** | the classes above — the only tab the `pdf` Startup Task writes |
...
Configure it on the plugin and register the task once:
...
It runs automatically after each development restart, walks every template published to `App_Data/pdf`
and writes one `.cs` per template, using the same
...
For per-template isolation, set `Namespace` on an existing model's `[Pdf]` attribute. The next generation
uses it for every class emitted from that template without changing the physical output folder:
...
The regenerated `[Pdf]` attribute retains the namespace override on subsequent runs.

**Take ownership explicitly.** Each generated file contains an opening `<auto-generated>` comment. While
that marker remains, the generator owns the file and may replace edits when its schema changes. Remove the
opening marker to preserve a hand-tuned file on future runs. Unmarked files are skipped and reported.
`Exclude` says the same thing up front, for a model you've adopted wholesale:
...
| `ModelsPath` | `<ServiceModel>/Pdf` | where the `pdf` Startup Task generates models |
