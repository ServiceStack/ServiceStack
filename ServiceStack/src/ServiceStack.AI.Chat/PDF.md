...
5. Configure `PdfCodeGen` and its `StartupTasks` registration, then populate the generated `[Pdf]` model.
...
register a development-only Startup Task:
...
It runs after each development restart and keeps the generated models synchronized with published templates:
...
Set `Namespace` on a template's existing `[Pdf]` model to override the namespace for every class generated
from that template without changing its physical output folder:
...
Each generated file carries an ownership marker:

```csharp
// <auto-generated>
//     Generated from App_Data/pdf/invoice.ui.json during development startup.
//     Remove this <auto-generated> line to preserve this file on future runs.
// </auto-generated>
#nullable enable
```

The marker means the generator owns the file and may replace it when its `.ui.json` schema changes. To
take ownership of a generated model, remove its opening `<auto-generated>` comment. Unmarked files are
skipped and reported, including hand-written files that happen to share the same name. `Exclude` says the
same thing up front, for a model you've adopted wholesale:
...
Restart the App in development to run its registered `pdf` Startup Task.
...
| `PreserveModified` | `true` | preserve existing files whose `<auto-generated>` marker was removed |
...
| `Namespace` | optional namespace override for C# classes generated from this template |
...
- Nothing in the Admin UI writes to the application's source tree. Only the explicitly registered `pdf`
  development Startup Task generates model source files.
...
2. Restart the App in development and review generated/unchanged/skipped output after schema changes.
...
| Generated file is not updated | its `<auto-generated>` marker was removed | restore the marker, exclude/adopt it intentionally, or set `PreserveModified = false` |
