...
{
    public static Task<TResponse> Execute<TRequest, TResponse>(this ChannelBase channel, TRequest request,
        string servicesName, string methodName,
        CallOptions options = default, string? host = null)
        where TRequest : class
        where TResponse : class
        => Execute<TRequest, TResponse>(channel.CreateCallInvoker(), request, servicesName, methodName, options,
            host);
    ...
    public static HttpClientHandler AddPemCertificateFromFile(this HttpClientHandler handler, string fileName,
        ...
        handler.AddPemCertificate(X509CertificateLoader.LoadCertificateFromFile(fileName), serverCertificateCustomValidationCallback);
#else
    ...
    public static CallOptions Init(this CallOptions options, GrpcClientConfig config, bool noAuth, CancellationToken token = default)
    {
        if (token != default)
        ...
        if (config.Headers.Count > 0 || auth != null || config.UserAgent != null)
        {
            ...
            if (auth != null)
            {
                if (auth == nameof(config.SessionId) && config.SessionId != null)
    ...
    public static async Task<(TResponse?, ResponseStatus?, Metadata)> GetResponseAsync<TResponse>(GrpcClientConfig config, AsyncUnaryCall<TResponse> auc)
    {
        ...
        if (typedStatus == null && status != null)
        {
            typedStatus = new ResponseStatus {
                ErrorCode = errorCode ?? TypeProperties.Get(status.GetType()).GetPublicGetter(nameof(ResponseStatus.ErrorCode))?.Invoke(status) as string ?? string.Empty,
                Message = message ?? TypeProperties.Get(status.GetType()).GetPublicGetter(nameof(ResponseStatus.Message))?.Invoke(status) as string,
            };
        }
