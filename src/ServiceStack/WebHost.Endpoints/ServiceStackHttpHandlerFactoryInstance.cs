...
{
	public class ServiceStackHttpHandlerFactoryInstance
		: IHttpHandlerFactory
	{
		readonly List<string> WebHostRootFileNames = new List<string>();
		...
		private readonly IHttpHandler DefaultHttpHandler = null;
		private readonly RedirectHttpHandler NonRootModeDefaultHttpHandler = null;
		private readonly IHttpHandler ForbiddenHttpHandler = null;
		private readonly IHttpHandler NotFoundHttpHandler = null;
		private readonly IHttpHandler StaticFileHandler = new StaticFileHandler();
		...
		private Func<IHttpRequest, IHttpHandler>[] RawHttpHandlers;

		[ThreadStatic]
		public static string DebugLastHandlerArgs;

		private EndpointHostInstance _endpointHost;
		...
		{
			return _endpointHost.Config.WebHostUrl ?? ApplicationBaseUrl;
		}

		// Entry point for HttpListener
		public IHttpHandler GetHandler(IHttpRequest httpReq)
		{
			foreach (var rawHttpHandler in RawHttpHandlers)
			{
				var reqInfo = rawHttpHandler(httpReq);
		...
		/// <returns></returns>
		private IHttpHandler ReturnRequestInfo(HttpRequest request)
		{
			...
			if (_endpointHost.Config.DebugOnlyReturnRequestInfo)
			{
				...
				reqInfo.Host = _endpointHost.Config.DebugAspNetHostEnvironment + "_v" + Env.ServiceStackVersion + "_" + _endpointHost.Config.ServiceName;
		...
		private bool ShouldAllow(string filePath)
		{
			var fileExt = Path.GetExtension(filePath);
			if (string.IsNullOrEmpty(fileExt)) return false;
			return _endpointHost.Config.AllowFileExtensions.Contains(fileExt.Substring(1));
		}

		public IHttpHandler GetHandlerForPathInfo(string httpMethod, string pathInfo, string requestPath, string filePath)
		{
		...
		private IHttpHandler GetCatchAllHandlerIfAny(string httpMethod, string pathInfo, string filePath)
		{
			if (_endpointHost.CatchAllHandlers != null)
			{
				foreach (var httpHandlerResolver in _endpointHost.CatchAllHandlers)
				{
