...
<script type=""text/javascript"">
    jQuery(function() {{
		...
		private static string ResultsJson(IHttpResponse httpRes, MiniProfiler profiler)
        {
			httpRes.ContentType = "application/json";
            return MiniProfiler.ToJson(profiler);
        }

		private static string ResultsFullPage(IHttpResponse httpRes, MiniProfiler profiler)
        {
        ...
        private static string GetResource(string filename)
        {
            ...
            {
                using (var stream = typeof(MiniProfilerHandler).Assembly.GetManifestResourceStream("ServiceStack.MvcMiniProfiler.UI." + filename))
