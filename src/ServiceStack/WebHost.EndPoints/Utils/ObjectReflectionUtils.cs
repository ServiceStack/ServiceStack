...
namespace ServiceStack.Common.Host.Utils
{
    public class ObjectReflectionUtils
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(ObjectReflectionUtils));
        ...
        private static object CreateDefaultValue(Type type)
        {
            ...
            if (hasEmptyConstructor)
            {
            	...
            	Type[] interfaces = type.FindInterfaces
            	        (
            	            (t, critera) => t.IsGenericType && t.GetGenericTypeDefinition() == typeof(ICollection<>),
            	            null
            	        );
    	...
    	public static void Add<T>(ICollection<T> list) where T : new()
        {
            if (list == null) return;
            list.Add(new T());
        }
