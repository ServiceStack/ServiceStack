...
}    public void Exceptions_thrown_are_received_by_client_when_AlwaysSendBasicAuthHeader_is_false_and_client_is_authenticated()
    {
      try
      {
        var client = GetClientWithUserPassword();
        ...
        var authRequest = new Auth { UserName = UserName, Password = Password };
        client.Send<AuthResponse>(authRequest);
