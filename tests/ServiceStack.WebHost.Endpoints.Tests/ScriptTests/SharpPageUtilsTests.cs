...
namespace ServiceStack.WebHost.Endpoints.Tests.ScriptTests
{
    public class SharpPageUtilsTests
    {
        ...
        {
            Assert.That(SharpPageUtils.ParseTemplate("").Count, Is.EqualTo(0));
            var fragments = SharpPageUtils.ParseTemplate("<h1>title</h1>");
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter }}</h1>");
            Assert.That(fragments.Count, Is.EqualTo(3));
            ...
            fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter() }}</h1>");
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter(1) }}</h1>");
            Assert.That(fragments.Count, Is.EqualTo(3));
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter(1,2.2,'a',\"b\",true) }}</h1>");
            Assert.That(fragments.Count, Is.EqualTo(3));
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter1 | filter2(1) | filter3(1,2.2,'a',\"b\",true) }}</h1>");
            Assert.That(fragments.Count, Is.EqualTo(3));
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("<h1>{{ title | filter1 }}</h1>\n<p>{{ content | filter2(a) }}</p>");
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("{{ filter }}");
            Assert.That(fragments.Count, Is.EqualTo(1));
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("{{ ' - {{it}}' | forEach(items) | markdown }}");
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("{{ array(['a',1,'c']) }}");
        ...
        public void Can_parse_templates_within_literals()
        {
            ...
            var fragments = SharpPageUtils.ParseTemplate("<ul>{{ '<li>{{it}}</li>' }}</ul>");
            Assert.That(fragments.Count, Is.EqualTo(3));
        }
        ...
        {
            var fragments1 = SharpPageUtils.ParseTemplate(
                @"{{ 
products 
  ...
  | select: { it.productName | raw } is sold out!\n 
            ...
            var fragments2 = SharpPageUtils.ParseTemplate(
            @"{{ products 
                 | where: it.UnitsInStock = 0 
                 | select: { it.productName | raw } is sold out!\n }}");
            
            // i.e. is rewritten and is equivalent to:
            var fragments3 = SharpPageUtils.ParseTemplate(
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate(
...
//                "{{ [c.CustomerId, o.OrderId, o.OrderDate] | jsv }}\n");
        ...
        public void Can_detect_invalid_syntax()
        {
            try
            {
                var fragments = SharpPageUtils.ParseTemplate("{{ arg | filter(' 1) }}");
            ...
            try
            {
                var fragments = SharpPageUtils.ParseTemplate("square = {{ 'square-partial | partial({ ten }) }}");
                Assert.Fail("should throw");
            }
            catch (ArgumentException e)
            {
            ...
            try
            {
                var fragments = SharpPageUtils.ParseTemplate("{{ arg | filter({ unterminated:1) }}");
                Assert.Fail("should throw");
            }
            catch (ArgumentException e)
            {
        ...
        {
            var fragments = SharpPageUtils.ParseTemplate("{{ 'foo' | assignTo: bar }}\n{{ bar }}");
            Assert.That(fragments.Count, Is.EqualTo(2));
            fragments = SharpPageUtils.ParseTemplate("{{ 'foo' | assignTo: bar }}\r\n{{ bar }}");
            Assert.That(fragments.Count, Is.EqualTo(2));

            fragments = SharpPageUtils.ParseTemplate("{{ ['foo'] | do: assign('bar', it) }}\n{{ bar }}");
            Assert.That(fragments.Count, Is.EqualTo(2));
            fragments = SharpPageUtils.ParseTemplate("{{ do: assign('bar', 'foo') }}\n{{ bar }}");
            Assert.That(fragments.Count, Is.EqualTo(2));
            fragments = SharpPageUtils.ParseTemplate("{{ 10 | times | do: assign('bar', 'foo') }}\n{{ bar }}");
            Assert.That(fragments.Count, Is.EqualTo(2));
            fragments = SharpPageUtils.ParseTemplate("{{ 10 | times | do: assign('bar', 'foo') }}\nbar");
            Assert.That(fragments.Count, Is.EqualTo(2));
