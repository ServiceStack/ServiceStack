...
namespace ServiceStack.WebHost.Endpoints.Tests.TemplateTests
{
    public class EvaluateScriptTests
    {
        ...
        public void Evaluate_does_convert_return_Value()
        {
            ...
            Assert.That(context.Evaluate("{{ return({Id:1,Name:'foo'}) }}"), Is.EqualTo(new Dictionary<string, object> {
                {"Id", 1},
                {"Name", "foo"},
            }));

            var person = context.Evaluate<Person>("{{ return({Id:1,Name:'foo'}) }}");
            ...
            Assert.That(context.Evaluate<Dictionary<string, object>>("{{ return(person) }}", new ObjectDictionary {
                ["person"] = new Person { Id = 1, Name = "foo" }
