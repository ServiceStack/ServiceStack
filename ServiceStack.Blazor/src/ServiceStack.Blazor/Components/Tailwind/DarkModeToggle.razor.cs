...
public partial class DarkModeToggle : UiComponentBase
{
    ...
    async Task toggleDark()
    {
        if (BlazorConfig.Instance.ToggleDarkMode())
        {
            ...
            await JS.InvokeVoidAsync("JS.elInvokeObjectMethod", ":root", "style", "setProperty", new[] { "color-scheme", "dark" });
        }
        else
        {
            await LocalStorage.RemoveAsync(Key);
            await JS.InvokeVoidAsync("JS.removeClass", "html", "dark");
            await JS.InvokeVoidAsync("JS.elInvokeObjectMethod", ":root", "style", "setProperty", new[] { "color-scheme", null });
        }
        StateHasChanged();
    }
