...
{
	[TestFixture]
	public class SimpleCacheAccessUsingConfigTests : CacheTestsBase
	{
		...
		public void AddAndGetSimpleObject()
		{
			...
			this.ConfigCache.Store(StoreMode.Add, simpleObject.Id.ToString(), simpleObject, DateTime.Now.AddHours(1));
		...
		{
			var simpleObject = new SimpleObject { Id = Guid.NewGuid() };
			this.ConfigCache.Store(StoreMode.Set, simpleObject.Id.ToString(), simpleObject, DateTime.Now.AddHours(1));
		...
		{
			var simpleObject = new SimpleObject { Id = Guid.NewGuid() };
			this.ConfigCache.Store(StoreMode.Replace, simpleObject.Id.ToString(), simpleObject, DateTime.Now.AddHours(1));
