...
sed -i "s/\"version\": \"[^\"]\+\"/\"version\": \"${Version}\"/g" ./src/ServiceStack.Api.OpenApi/project.json
...
find ./NuGet.Core -type f -name "*.nuspec" -exec sed -i "s/\"ServiceStack.Api.OpenApi.Core\" version=\"[^\"]\+\"/\"ServiceStack.Api.OpenApi.Core\" version=\"\[${PackageVersion}, \)\"/g" {} +
