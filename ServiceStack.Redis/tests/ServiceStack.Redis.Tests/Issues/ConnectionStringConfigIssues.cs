...
namespace ServiceStack.Redis.Tests.Issues
{
    public class ConnectionStringConfigIssues
    {
        ...
        {
            var connString = "127.0.0.1?password=" + "p@55wOrd=".UrlEncode();

            var config = connString.ToRedisEndpoint();
            Assert.That(config.Password, Is.EqualTo("p@55wOrd="));
        }

        [Test, Ignore("Requires redis-server configured with 'requirepass p@55wOrd='")]
        public void Can_connect_to_redis_with_password_with_equals()
        {
            ...
            {
                Assert.That(redis.Password, Is.EqualTo("p@55wOrd="));
            }
        }
    }
}